...
void CIrrDeviceMacOSX::postKeyEvent(void *event,irr::SEvent &ievent,bool pressed)
{
	...
	unsigned int mkey = 0, mchar = 0;

	str = [(NSEvent *)event characters];
	int code = [(NSEvent *)event keyCode];
	...
	iter = KeyCodes.find(code);
	if (iter != KeyCodes.end())
		mkey = (*iter).second;
	else
	{
		mkey = 0;
		NSLog(@"unmmaped keycode, please extend KeyCodes tables.  Code is 0x%x", code);
	}
...
void CIrrDeviceMacOSX::initKeycodes()
{
	...
	KeyCodes[kVK_ISO_Section] = irr::KEY_OEM_3;
	KeyCodes[kVK_ANSI_Semicolon] = irr::KEY_OEM_1;
	KeyCodes[kVK_ANSI_LeftBracket] = irr::KEY_OEM_4;
	KeyCodes[kVK_ANSI_RightBracket] = irr::KEY_OEM_6;
	KeyCodes[kVK_ANSI_Quote] = irr::KEY_OEM_4;
	KeyCodes[kVK_ANSI_Backslash] = irr::KEY_OEM_102;
	KeyCodes[kVK_ANSI_Equal] = irr::KEY_PLUS;
	...
	KeyCodes[kVK_ANSI_KeypadPlus] = irr::KEY_ADD;
